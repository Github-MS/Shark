...
\noindent\fbox{%
    \parbox{\textwidth}{%
\underline{Server information:}\\

\textbf{IP address:} 145.88.76.243\\
\textbf{Hostname:} res-hpc-lo01\\
\textbf{Port:} 22}%
}
...
**Connect with Putty**
    ...
    5. (Optional) At the connection setting, fill in 60 at the Seconds between keepalives. If needed, enable the Enable TCP keepalives.
    6. (Optional) At the X11 setting, you can enable the Enable X11 forwarding. Do this if you need graphical output, but for this to get this working, you need to install separately a X11 server for Windows.
...
First option (the session is saved for future connections):
    ...
    2. Click on sessions.
    3. Click on ssh.
    4. Fill in the 'Remote host' (res-hpc-lo01 or 145.88.76.243).
...
Second option (faster):
    ...
    3. Type: ssh <SHARKusername>@145.88.76.243 .
...
First option (recommended):
    ...
    2. Create a config file inside your .ssh directory and add the following: vi ~/.ssh/config (only the first time)
    ...
    5. Set the permissions for the user on all the files in your .ssh directory: chmod 600 ~/.ssh/* (only the first time)
    6. Type ssh <SHARKUsername>@res-hpc-lo01
...
Second option (faster):
    ...
    1. Open the terminal.
    2. Type: ssh -XY <SHARKusername>@res-hpc-lo01
...
First option (the session is saved for future connections):
    ...
    2. Click on sessions.
    3. Click on ssh.
    4. Type 'res-hpc-lo01' or '145.88.76.243' in 'Remote host'.
    5. Select specify username and introduce your SHARKusername, and set port to 22.
    6. (Optional) Check the 'Follow SSH path' in the 'Advanced SSH settings'-TAB.
...
Second option (faster)
    ...
    4. Introduce your password.
    5. Type: ssh res-hpc-lo01 or ssh 145.88.76.243 .
...
Before connecting the first time follow the next steps:
    ...
    3. Add also:
      Host resshark
        Hostname 145.88.76.243
        Localforward 6001 145.88.76.243:22
        User <SHARKUsername>
...
First **Filezilla** is installed. Fill the host (145.88.76.243), the username (SHARKusername), the password (SHARKpassword) and the port (22) as in Figure 2 (Watch out! The host name of the image is not the correct one), and press the ENTER key. 
...
scp -r mydirectory SHARKusername@res-hpc-lo01:/exports/fsw

To \underline{download} files/folder from SHARK just interchange the paths: 

**scp SHARKusername@res-hpc-lo01:/exports/fsw/myfile.txt MyDocuments**
...
**scp text.txt SHARKusername@res-hpc-lo01:/home/SHARKusername**
...
**scp SHARKusername@res-hpc-lo01:/home/SHARKusername/text.txt /home/SHARKusername**
...
Start by install Fugu. Then, open Fugu and in the menu go to SSH and afterward New SSH tunnel (cmd T). Fill Create Tunnel to (145.88.76.243), Service or Port (22), Tunnel Host(145.88.35.10), Username (SHARKusername) and Port (22).
...
First **Filezilla** is installed. Fill the host (145.88.76.243), the username (SHARKusername), the password (SHARKpassword) and the port (22) as in Figure 2, and press the ENTER key. 
...
Let's suppose that we want to submit all the replications for the first row of our design matrix. The code should be as follow:

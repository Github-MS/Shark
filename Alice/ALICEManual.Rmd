...
If you are using **Putty** to connect to ALICE, the process is far more difficult. It is necessary to download a software such as WinSCP and create an SSH tunnel.
...
**Remark**: In ALICE, the files must be transfered through the jump host. This means that you need to transfer the files from your PC/laptop to the jump host using scp and then from the jump host to ALICE (or vice versa if you want to download files from ALICE). Let's see how to do it with two examples:
...
For your own simulation study, we advise you to start by running only the row of your design which you expect to require more memory. If 512MB is enough there is no need to modify the script to run all the jobs. In case the memory is not enough (ALICE will return an error), increase it gradually, for example, with increments of 0.5Gb. Asking for too much memory will require a node with more memory, and consequently, your job will be more time in the queue. Also, make sure to select the right partition for your job. cpu-short (max 4 hours), cpu-medium (max 1 day) and cpu-long (max 7 days) are the partitions available for jobs that do not require a GPU. If you need a GPU, you can select the partition gpu-short, gpu-medium or gpu-long. Other partitions are available. Check https://pubappslu.atlassian.net/wiki/spaces/HPCWIKI/pages/37519504/Partitions+on+ALICE
...
**Note 2:** *In order to use R on ALICE, you first need to 'load' it. This can simply be done on the command line by typing module load R/4.4.0-gfbf-2023a (or select another R version available on ALICE)*
...
```
$ salloc -N1 -n1
$ module load R/4.4.0-gfbf-2023a (or another version)
$ R
```
...
mkdir /home/YOUR_DIRECTORY/R/x86_64-pc-linux-gnu-library/4.4 (or the corresponding R version)
